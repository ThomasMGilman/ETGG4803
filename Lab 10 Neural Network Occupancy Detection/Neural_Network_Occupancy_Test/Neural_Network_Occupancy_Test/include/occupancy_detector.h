...
private:
	...
	int mNeuralNetwork_trainingTimes = 1;
	unsigned numTests = 0;
	...
	vector<float> mClassification_percentages;
	...
	string mDumpDataPath;
protected:
	NeuralNetwork* mNetwork_classifier;
...
public:
	...
	OccupancyDetector(int NeuralNetwork_trainingTimes, int hidden_neurons, int hidden_layers, float neurlTrainRate,
		string training_data_folder, string test_data_folder, bool outputData, string outPutPath = "");

	~OccupancyDetector() 
	{
		delete(mNetwork_classifier);
		delete(mOcc);
		delete(mUnocc);
...
public:
	...
	void output_to_file(vector<OccupancyTest*> data_set, string file_name, string file_path);

	std::vector<float> get_percentage_right() { return mClassification_percentages; };
