...
{
	double g_x = sigmoid_logistic(z);
	//double natLog_negInput = exp(-z);
	return g_x * (1 - g_x);//natLog_negInput / pow(natLog_negInput + 1, 2);
}

double hyperbolic_tan(double x)
{
	double nat_negX = exp(-x), nat_posX = exp(x);
	return (nat_posX - nat_negX) / (nat_posX + nat_negX);
}
...
{
	return (sensitivity * testPositive) / (sensitivity * testPositive + (1 - specificity) * (1 - testPositive));
}
