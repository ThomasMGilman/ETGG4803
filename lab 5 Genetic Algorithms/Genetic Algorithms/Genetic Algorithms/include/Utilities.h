...
public:
	...
	template<typename T>
	T roulette_wheel_selection(vector<T> data, T maxVal)
	{
	...
	template<typename T>
	chromosome<T> create_chromosome(vector<T>& sequenceToCreateFrom, chromosome<T>& sequenceToMatch, int sizeOfChromosomeData)
	{
		chromosome<T> c;
		c.sequence = create_chromosome_data<T>(sequenceToCreateFrom, sizeOfChromosomeData);
		c.matches = get_match_count<T>(c.sequence, sequenceToMatch);
		return c;
	}

	template<typename T>
	bool random_reset_check(chromosome<T>& c, chromosome<T>& sequenceToMatch, vector<T>& sequenceToCreateFrom, int sizeOfChromosomeData, int randResetChanceRange, int randResetChance)
	{
		int chance = rand() % randResetChanceRange + 1;
		if (chance <= randResetChance)
		{
			c = create_chromosome<T>(sequenceToCreateFrom, sequenceToMatch, sizeOfChromosomeData);
			return true;
		}
		return false;
	}
	...
	template<typename T>
	void get_parent(chromosome<T>& parent, chromosome<T>& sequenceToMatch, vector<chromosome<T>>* lastGenParents, vector<T>& sequenceToCreateFrom, 
		int sizeOfChromosomeData, int parentsToKeep, int offsetIntoParents,
		...
		int randResetChanceRange, int randResetChance)
	{
		...
		else if (!random_reset_check<T>(parent, sequenceToMatch, sequenceToCreateFrom, sizeOfChromosomeData, randResetChanceRange, randResetChance))
		{
			chance = rand() % randWheelSpinChanceRange + 1;
			if (chance <= randWheelSpinChance)
			{
				vector<chromosome<T>> copy = *lastGenParents;
				parent = roulette_wheel_selection<chromosome<T>>(copy, sequenceToMatch);
			}
			else
			{
