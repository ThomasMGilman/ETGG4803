...
template<typename T>
double dot(vector<T> vecA, vector<T> vecB)
{
	double result = 0;
...
/* Logistic function 1 / (1+e^-z) used for creating a soft threshold*/
double sigmoid_logistic(double z);

/* sigmoid derivative = e^-z / (e^-z + 1)^2 */
double sigmoid_derivative(double z);

/// Shifted and scaled version of sigmoid_logistic
double hyperbolic_tan(double x);

double gudermannian_func(double x);
...
template<typename T>
double calculate_mean(vector<T>& values)
{
...
template<typename T>
double calculate_standard_deviation(vector<T>& values, T mean)
{
	double deviation_amnt = 0;
